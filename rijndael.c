...
#include "rijndael.h"
/*
    COMMON FUNCTIONS (Functions that are used in both encryption and decryption)
 */
...
void KeyExpansion(unsigned char* inputKey, unsigned char* expandedKeys) {
	...
	int bytesGenerated = BLOCK_SIZE;
	int rcon_location = 1;
	unsigned char key_block[4];


	while (bytesGenerated < (BLOCK_SIZE * (NUM_ROUNDS + 1))) {
		for (int i = 0; i < ((BLOCK_SIZE * 8) / 32); i++) { //while is less than the number of 32 bit words in 176 bit expanded keys
			key_block[i] = expandedKeys[i + bytesGenerated - ((BLOCK_SIZE * 8) / 32)];
		}
		...
		for (int i = 0; i < 4; i++) {
			expandedKeys[bytesGenerated] = expandedKeys[bytesGenerated - BLOCK_SIZE] ^ key_block[i];
			bytesGenerated++;
		}
	}
}
...
void ShiftRows(unsigned char * plain_text) {
	...
	for (int i = 0; i < BLOCK_SIZE; i += 4) {
		...
		temp_block[i + 1] = plain_text[(i + 5) % BLOCK_SIZE];
		temp_block[i + 2] = plain_text[(i + 10) % BLOCK_SIZE];
		temp_block[i + 3] = plain_text[(i + 15) % BLOCK_SIZE];
	}
...
void AES_encrypt(unsigned char * plain_text, unsigned char * key) {
	...
	unsigned char expandedKey[BLOCK_SIZE * (NUM_ROUNDS + 1)]; 
	...
	for (int i = 0; i < NUM_ROUNDS - 1; i++) {
		...
		AddRoundKey(plain_text, expandedKey + (BLOCK_SIZE * (i + 1)));
	}

	SubBytes(plain_text);
	ShiftRows(plain_text);
	AddRoundKey(plain_text, expandedKey + (BLOCK_SIZE * NUM_ROUNDS));
}


void aes_ecb_encrypt(unsigned char * plain_text, unsigned char* key, int text_length) {

	for (int i = 0; i < text_length; i += BLOCK_SIZE) {
...
void ReverseShiftRows(unsigned char * plain_text) {
	...
	for (int i = 0; i < BLOCK_SIZE; i += 4) {
		//incrementing by 5 causes the diagonal shift effect
		temp_block[i] = plain_text[i];
		temp_block[(i + 5) % BLOCK_SIZE] = plain_text[i+1];
		temp_block[(i + 10) % BLOCK_SIZE] = plain_text[i+2];
		temp_block[(i + 15) % BLOCK_SIZE] = plain_text[i+3];
	}
...
void AES_decrypt(unsigned char * plain_text, unsigned char * key) {
	...
	for (int i = NUM_ROUNDS -1; i >= 1; i--) {
		AddRoundKey(plain_text, expandedKey + (BLOCK_SIZE * i));
...
void aes_cbc_decrypt(unsigned char * plain_text, unsigned char* key, int text_length, unsigned char * iv) {
	...
	unsigned char * xor_vector = iv;
	unsigned char place_holder[BLOCK_SIZE];

	for (int i = 0; i < text_length; i += BLOCK_SIZE) {
		memcpy(place_holder, plain_text+i, BLOCK_SIZE);
		AES_decrypt(plain_text + i, key);
		AddRoundKey(plain_text + i, xor_vector);
		memcpy(xor_vector, place_holder, BLOCK_SIZE);
	}
}
